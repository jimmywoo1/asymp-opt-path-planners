...
RESULTS_PATH = "../../results/nrrt_results.csv"
...
class NRRTPlanner:
    ...
    def display_prob_map(self):
        ...
        ax1 = sns.heatmap(self.prob, ax=axes[0], cmap=sns.color_palette("Blues", as_cmap=True), cbar=False, xticklabels=False, yticklabels=False)
    ...
    def uniform_sample(self, sample_size):
        ...
        return [State(a, b, None) for b, a in zip(ys, xs)]
    ...
    def plan(self, start, end, max_iterations, max_steering_radius, tolerance, sample_size=5):
        ...
        for step in range(max_iterations):
            ...
            if random.random(sample_size) < self.alpha else self.non_uniform_sample(sample_size)
        ...
        self.complete_tree = tree_nodes

        return plan, cost, step, self.num_collision_checks

    def plot_tree(self, plan, path):
        ...
        draw_plan(img, plan, bgr=(0,0,255), thickness=2, file_path=path)
...
def simulation(planner, start, end, max_num_steps, max_steering_radius, batch_size, check_div):
    ...
    return plan, cost, num_iterations, end_time, num_collision_checks
